...
选择数据库轮询和 goroutine，减少外部依赖。每秒读一批 pending，以带旧状态条件的 UPDATE 领取，RowsAffected=1 才执行。读取批大小100不代表并发上限；通过 WORKER_CONCURRENCY 限制整个转写和摘要过程的并发，默认3；满载时任务留在 pending，名额释放后由后续轮询领取。非法配置拒绝启动。
...
不提供真实 ASR、自动重试、SSE、上传幂等、鉴权、前端、分布式协调或公网部署。
